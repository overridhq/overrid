#!/usr/bin/env python3
"""Run the Overrid validation suite configured for local repo checks."""

from __future__ import annotations

from pathlib import Path
import subprocess
import sys


REPO_ROOT = Path(__file__).resolve().parents[1]
VALIDATORS = [
    Path("scripts/validate_admin_ui.py"),
    Path("scripts/validate_repository_layout_phase1.py"),
    Path("scripts/validate_repository_layout_phase2.py"),
    Path("scripts/validate_repository_layout_phase3.py"),
    Path("scripts/validate_repository_layout_phase4.py"),
    Path("scripts/validate_repository_layout_phase5.py"),
    Path("scripts/validate_repository_layout_phase6.py"),
    Path("scripts/validate_repository_layout_phase7.py"),
    Path("scripts/validate_repository_layout_phase8.py"),
    Path("scripts/validate_repository_layout_phase9.py"),
    Path("scripts/validate_repository_layout_phase10.py"),
    Path("scripts/validate_integration_harness_phase1.py"),
    Path("scripts/validate_integration_harness_phase2.py"),
    Path("scripts/validate_integration_harness_phase3.py"),
    Path("scripts/validate_integration_harness_phase4.py"),
    Path("scripts/validate_integration_harness_phase5.py"),
    Path("scripts/validate_integration_harness_phase6.py"),
    Path("scripts/validate_integration_harness_phase7.py"),
    Path("scripts/validate_integration_harness_phase8.py"),
    Path("scripts/validate_integration_harness_phase9.py"),
    Path("scripts/validate_integration_harness_phase10.py"),
    Path("scripts/validate_local_development_stack_phase1.py"),
    Path("scripts/validate_local_development_stack_phase2.py"),
    Path("scripts/validate_local_development_stack_phase3.py"),
    Path("scripts/validate_local_development_stack_phase4.py"),
    Path("scripts/validate_local_development_stack_phase5.py"),
    Path("scripts/validate_local_development_stack_phase6.py"),
    Path("scripts/validate_local_development_stack_phase7.py"),
    Path("scripts/validate_local_development_stack_phase8.py"),
    Path("scripts/validate_local_development_stack_phase9.py"),
    Path("scripts/validate_local_development_stack_phase10.py"),
    Path("scripts/validate_cli_phase1.py"),
    Path("scripts/validate_cli_phase2.py"),
    Path("scripts/validate_cli_phase3.py"),
    Path("scripts/validate_cli_phase4.py"),
    Path("scripts/validate_cli_phase5.py"),
    Path("scripts/validate_cli_phase6.py"),
    Path("scripts/validate_cli_phase7.py"),
    Path("scripts/validate_cli_phase8.py"),
    Path("scripts/validate_cli_phase9.py"),
    Path("scripts/validate_cli_phase10.py"),
    Path("scripts/validate_overgate_phase1.py"),
    Path("scripts/validate_overgate_phase2.py"),
    Path("scripts/validate_overgate_phase3.py"),
    Path("scripts/validate_overgate_phase4.py"),
    Path("scripts/validate_overgate_phase5.py"),
    Path("scripts/validate_overgate_phase6.py"),
    Path("scripts/validate_overgate_phase7.py"),
    Path("scripts/validate_overgate_phase8.py"),
    Path("scripts/validate_overgate_phase9.py"),
    Path("scripts/validate_overgate_phase10.py"),
    Path("scripts/validate_overkey_phase1.py"),
    Path("scripts/validate_overkey_phase2.py"),
    Path("scripts/validate_overkey_phase3.py"),
    Path("scripts/validate_overkey_phase4.py"),
    Path("scripts/validate_overkey_phase5.py"),
    Path("scripts/validate_overkey_phase6.py"),
    Path("scripts/validate_overkey_phase7.py"),
    Path("scripts/validate_sdk_phase1.py"),
    Path("scripts/validate_sdk_phase2.py"),
    Path("scripts/validate_sdk_phase3.py"),
    Path("scripts/validate_sdk_phase4.py"),
    Path("scripts/validate_sdk_phase5.py"),
    Path("scripts/validate_sdk_phase6.py"),
    Path("scripts/validate_sdk_phase7.py"),
    Path("scripts/validate_sdk_phase8.py"),
    Path("scripts/validate_sdk_phase9.py"),
    Path("scripts/validate_sdk_phase10.py"),
    Path("scripts/validate_shared_schema_package_phase1.py"),
    Path("scripts/validate_shared_schema_package_phase2.py"),
    Path("scripts/validate_shared_schema_package_phase3.py"),
    Path("scripts/validate_shared_schema_package_phase4.py"),
    Path("scripts/validate_shared_schema_package_phase5.py"),
    Path("scripts/validate_shared_schema_package_phase6.py"),
    Path("scripts/validate_shared_schema_package_phase7.py"),
    Path("scripts/validate_shared_schema_package_phase8.py"),
    Path("scripts/validate_shared_schema_package_phase9.py"),
    Path("scripts/validate_shared_schema_package_phase10.py"),
]


def main() -> int:
    for validator in VALIDATORS:
        path = REPO_ROOT / validator
        if not path.is_file():
            print(f"missing validator: {validator}", file=sys.stderr)
            return 1
        result = subprocess.run([sys.executable, str(path)], cwd=REPO_ROOT)
        if result.returncode != 0:
            return result.returncode
    print("Overrid validation suite passed.")
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
