#!/usr/bin/env python3
"""Validate Local Development Stack Phase 1 gate semantics."""
...
SERVICE = Path("docs/service_catalog/foundation/local_development_stack.md")
MASTER = Path("docs/build_plan/master_plan.md")
CROSSWALK = Path("docs/build_plan/service_catalog_alignment.md")
TECH_STACK = Path("docs/overrid_tech_stack_choice.md")
PHASE_PLAN = Path("docs/planning/local_development_stack_phase_01_plan.md")
PHASE_PROGRESS = Path("docs/planning/local_development_stack_phase_01_progress.md")
...
REQUIRED_BLOCKERS = [
    "`dependency.phase0_contract_missing`",
    "`dependency.phase1_control_plane_unavailable`",
    "`dependency.overcell_contract_missing`",
    "`dependency.execution_loop_contract_missing`",
    "`dependency.trust_policy_contract_missing`",
    "`dependency.accounting_contract_missing`",
    "`dependency.product_integration_contract_missing`",
    "`dependency.grid_resident_contract_missing`",
    "`dependency.native_storage_contract_missing`",
    "`dependency.deployment_contract_missing`",
    "`dependency.federation_contract_missing`",
    "`dependency.public_provider_contract_missing`",
    "`dependency.native_app_contract_missing`",
    "`dependency.governance_contract_missing`",
]

REQUIRED_BOUNDARY_TEXT = [
    "not a deployment orchestrator",
    "not a production control plane",
    "not a payment runner",
    "not a public-provider testbed",
...
REQUIRED_ATTACHMENT_PATHS = [
    ...
    "docs/planning/local_development_stack_phase_01_plan.md",
    "docs/planning/local_development_stack_phase_01_progress.md",
]

REQUIRED_RESOLVED_DECISIONS = [
    "Rust-owned embedded/local durable state",
    "Filesystem-backed content-addressed Overstore stub",
    "Exactly one local Overcell-like node simulator",
    "API `18080`",
    "worker health/metrics `18081`",
    "node-agent simulator `18082`",
    "object/artifact stub `18083`",
    "local event/audit query `18084`",
    "optional developer UI `18085`",
    "reproducible Linux x86_64",
]
...
def validate_sub_plan() -> None:
    text = read(SUB_PLAN)
    assert_contains(text, "# SUB BUILD PLAN #4 - Local Development Stack", SUB_PLAN)
    assert_contains(
        text,
        "Attached SDS: [docs/sds/foundation/local_development_stack.md]",
        SUB_PLAN,
    )
    ...
    for blocker in REQUIRED_BLOCKERS:
        assert_contains(phase_1, blocker, SUB_PLAN)
    ...
    for phase in range(0, 14):
        if not re.search(rf"^\| {phase} \|", phase_1, flags=re.MULTILINE):
            raise AssertionError(f"{SUB_PLAN} gate matrix is missing phase {phase}")
...
def validate_cross_doc_alignment() -> None:
    ...
    assert_contains(sds, "[sub_build_plan_004_local_development_stack.md]", SDS)
    ...
    assert_contains(service, "[SUB BUILD PLAN #4 - Local Development Stack]", SERVICE)
    assert_contains(service, "## Phase 1 Implementation Gates", SERVICE)
    for state in REQUIRED_GATE_STATES:
        assert_contains(service, state, SERVICE)
    assert_contains(service, "not a production control plane", SERVICE)
    assert_contains(service, "Direct storage, queue, object, event, or local file inspection is allowed only for reset, seed, health, and diagnostics", SERVICE)
    assert_contains(service, "Freeze Phase 1 SDS attachment", SERVICE)
    for decision in REQUIRED_RESOLVED_DECISIONS[:4]:
        assert_contains(service, decision, SERVICE)

    assert_contains(master, "SDS #4: [Local Development Stack]", MASTER)
    assert_contains(master, "[SUB BUILD PLAN #4 - Local Development Stack]", MASTER)
    assert_contains(master, "First build point remains Phase 0", MASTER)

    assert_contains(crosswalk, "| SDS #4 | [Local Development Stack]", CROSSWALK)
    assert_contains(crosswalk, "[SUB BUILD PLAN #4 - Local Development Stack]", CROSSWALK)
    ...
    assert_contains(phase_plan, "Complete SUB BUILD PLAN #4 Phase 1", PHASE_PLAN)
    assert_contains(phase_plan, "Do not introduce the Rust local-stack crate in this phase", PHASE_PLAN)
    assert_contains(phase_progress, "Local Development Stack Phase 1 Progress", PHASE_PROGRESS)
    assert_contains(phase_progress, "Validation Evidence", PHASE_PROGRESS)

    assert_contains(
        suite_validator,
        'Path("scripts/validate_local_development_stack_phase1.py")',
        SUITE_VALIDATOR,
    )
...
def main() -> int:
    ...
    for check in checks:
        check()
    print("Local Development Stack Phase 1 validation passed.")
    return 0
