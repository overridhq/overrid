...
| Sub-build plan | [SUB BUILD PLAN #18 - Node Installer](../../build_plan/sub_build_plan_018_node_installer.md) |
...
- Uninstall must remove service binaries, supervisor units, raw configs, local private keys, enrollment tokens, runtime caches, and sensitive logs, but preserve a minimal audit tombstone by default: install-session id, node id hash/ref, bundle and channel history, config hash, uninstall receipt, credential revocation ref, diagnostics manifest hashes, rollback refs, command transcript refs, and Overwatch audit refs. This retained record must contain no usable credentials or raw host secrets. A local purge of audit tombstones requires explicit operator confirmation and must be allowed only after control-plane evidence upload or an explicit offline evidence-loss record.
- Public provider diagnostics contain only safe summaries: installer version, bundle hash/signature refs, platform compatibility result, update channel, service status, heartbeat/discovery status, redacted config hash, reason codes, trace/install refs, coarse OS/runtime class, and redacted log summaries. Restricted operator evidence may include raw command transcript refs, exact platform/runtime details, local path refs, private network facts, host identifiers, route details, enrollment denial detail, and raw log refs only behind purpose-scoped authorization, audit, and retention controls. Tokens, private keys, secret env vars, unrelated logs, user files, and raw enrollment material are never included in either bundle.
- Installer channels map to signed bundle policy. `stable` is the default for founder seed, private-swarm, trusted-provider, and public-provider nodes. `beta` is explicit opt-in for test, pilot, or non-critical capacity and must not make a node eligible for higher-sensitivity or system-service placement by itself. `emergency_rollback` is a scoped, expiring, signed rollback channel that may only target a known-good compatible bundle, requires operator or policy authorization, emits high-severity Overwatch evidence, and is revoked once the incident window closes.
