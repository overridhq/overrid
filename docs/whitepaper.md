...
Payment-provider selection should remain an Overbill implementation decision. The default is fiat billing, invoices, tax support, refunds, chargeback evidence, and provider cash-out through compliant payment rails. Stablecoin or tokenized settlement is a separate regulated track, not the default, and it should never be required for normal ORU accounting or fee-free internal settlement.
